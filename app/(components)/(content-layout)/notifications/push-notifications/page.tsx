...
const PushNotifications = () => {
    ...
    const pushNotificationsData = [
        { id: 1, name: 'Push Notification #001', createdDate: '2024-01-15', status: 'Active' },
        { id: 2, name: 'Push Notification #002', createdDate: '2024-01-10', status: 'Active' },
        { id: 3, name: 'Push Notification #003', createdDate: '2024-01-08', status: 'Inactive' },
    ]

    return (
        <Fragment>
            <Seo title="Push Notifications" />
            <Pageheader title="Notifications" subtitle="Push Notifications" currentpage="Push Notifications" activepage="Push Notifications" />
                            ...
                            <div className="d-flex align-items-center justify-content-between flex-wrap gap-3">
                                <div className="d-flex flex-wrap gap-1">
                                    <SpkButton Buttonvariant="primary" Customclass="btn me-2">
                                        <i className="ri-add-line me-1 fw-medium align-middle"></i>Add Push Notification
                                    </SpkButton>
                                </div>
                        ...
                        <Card.Body className="p-0">
                            <div className="table-responsive">
                                <SpkTables tableClass="text-nowrap" header={[{ title: 'ID' }, { title: 'Name' }, { title: 'Created Date' }, { title: 'Status' }, { title: 'Actions' }]}>
                                    {pushNotificationsData.map((notification, index) => (
                                        <tr key={index}>
                                            <td>{notification.id}</td>
                                            <td>{notification.name}</td>
                                            <td>{notification.createdDate}</td>
                                            <td>
                                                <span className={`badge ${notification.status === 'Active' ? 'bg-success-transparent' : 'bg-danger-transparent'}`}>
                                                    {notification.status}
                                                </span>
                                            </td>
                                            <td>
