...
const DeviceTokens = () => {
    ...
    const deviceTokensData = [
        { id: 1, name: 'Device Token #001', createdDate: '2024-01-15', status: 'Active' },
        { id: 2, name: 'Device Token #002', createdDate: '2024-01-10', status: 'Active' },
        { id: 3, name: 'Device Token #003', createdDate: '2024-01-08', status: 'Inactive' },
    ]

    return (
        <Fragment>
            <Seo title="Device Tokens" />
            <Pageheader title="Notifications" subtitle="Device Tokens" currentpage="Device Tokens" activepage="Device Tokens" />
                            ...
                            <div className="d-flex align-items-center justify-content-between flex-wrap gap-3">
                                <div className="d-flex flex-wrap gap-1">
                                    <SpkButton Buttonvariant="primary" Customclass="btn me-2">
                                        <i className="ri-add-line me-1 fw-medium align-middle"></i>Add Device Token
                                    </SpkButton>
                                </div>
                        ...
                        <Card.Body className="p-0">
                            <div className="table-responsive">
                                <SpkTables tableClass="text-nowrap" header={[{ title: 'ID' }, { title: 'Name' }, { title: 'Created Date' }, { title: 'Status' }, { title: 'Actions' }]}>
                                    {deviceTokensData.map((token, index) => (
                                        <tr key={index}>
                                            <td>{token.id}</td>
                                            <td>{token.name}</td>
                                            <td>{token.createdDate}</td>
                                            <td>
                                                <span className={`badge ${token.status === 'Active' ? 'bg-success-transparent' : 'bg-danger-transparent'}`}>
                                                    {token.status}
                                                </span>
                                            </td>
                                            <td>
