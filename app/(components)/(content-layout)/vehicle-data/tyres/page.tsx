...
const Tyres = () => {
    ...
    const tyresData = [
        { id: 1, name: 'Tyre #001', createdDate: '2024-01-15', status: 'Active' },
        { id: 2, name: 'Tyre #002', createdDate: '2024-01-10', status: 'Active' },
        { id: 3, name: 'Tyre #003', createdDate: '2024-01-08', status: 'Inactive' },
    ]

    return (
        <Fragment>
            <Seo title="Tyres" />
            <Pageheader title="Admin" subtitle="Masters" currentpage="Tyres" activepage="Tyres" />
                            ...
                            <div className="d-flex align-items-center justify-content-between flex-wrap gap-3">
                                <div className="d-flex flex-wrap gap-1">
                                    <SpkButton Buttonvariant="primary" Customclass="btn me-2">
                                        <i className="ri-add-line me-1 fw-medium align-middle"></i>Add Tyre
                                    </SpkButton>
                                </div>
                        ...
                        <Card.Body className="p-0">
                            <div className="table-responsive">
                                <SpkTables tableClass="text-nowrap" header={[{ title: 'ID' }, { title: 'Name' }, { title: 'Created Date' }, { title: 'Status' }, { title: 'Actions' }]}>
                                    {tyresData.map((tyre, index) => (
                                        <tr key={index}>
                                            <td>{tyre.id}</td>
                                            <td>{tyre.name}</td>
                                            <td>{tyre.createdDate}</td>
                                            <td>
                                                <span className={`badge ${tyre.status === 'Active' ? 'bg-success-transparent' : 'bg-danger-transparent'}`}>
                                                    {tyre.status}
                                                </span>
                                            </td>
                                            <td>
