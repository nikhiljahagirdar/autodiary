...
const AppVersions = () => {
    ...
    const appVersionsData = [
        { id: 1, name: 'Version 1.2.3', createdDate: '2024-01-15', status: 'Active' },
        { id: 2, name: 'Version 1.2.2', createdDate: '2024-01-10', status: 'Active' },
        { id: 3, name: 'Version 1.2.1', createdDate: '2024-01-08', status: 'Inactive' },
    ]

    return (
        <Fragment>
            <Seo title="App Versions" />
            <Pageheader title="Admin" subtitle="Masters" currentpage="App Versions" activepage="App Versions" />
                            ...
                            <div className="d-flex align-items-center justify-content-between flex-wrap gap-3">
                                <div className="d-flex flex-wrap gap-1">
                                    <SpkButton Buttonvariant="primary" Customclass="btn me-2">
                                        <i className="ri-add-line me-1 fw-medium align-middle"></i>Add App Version
                                    </SpkButton>
                                </div>
                        ...
                        <Card.Body className="p-0">
                            <div className="table-responsive">
                                <SpkTables tableClass="text-nowrap" header={[{ title: 'ID' }, { title: 'Name' }, { title: 'Created Date' }, { title: 'Status' }, { title: 'Actions' }]}>
                                    {appVersionsData.map((app, index) => (
                                        <tr key={index}>
                                            <td>{app.id}</td>
                                            <td>{app.name}</td>
                                            <td>{app.createdDate}</td>
                                            <td>
                                                <span className={`badge ${app.status === 'Active' ? 'bg-success-transparent' : 'bg-danger-transparent'}`}>
                                                    {app.status}
                                                </span>
                                            </td>
                                            <td>
