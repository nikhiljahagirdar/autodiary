...
const ReminderTypes = () => {
    ...
    const reminderTypesData = [
        { id: 1, name: 'Insurance Renewal', createdDate: '2024-01-15', status: 'Active' },
        { id: 2, name: 'Service Due', createdDate: '2024-01-10', status: 'Active' },
        { id: 3, name: 'Registration Renewal', createdDate: '2024-01-08', status: 'Inactive' },
    ]

    return (
        <Fragment>
            <Seo title="Reminder Types" />
            <Pageheader title="Admin" subtitle="Masters" currentpage="Reminder Types" activepage="Reminder Types" />
                            ...
                            <div className="d-flex align-items-center justify-content-between flex-wrap gap-3">
                                <div className="d-flex flex-wrap gap-1">
                                    <SpkButton Buttonvariant="primary" Customclass="btn me-2">
                                        <i className="ri-add-line me-1 fw-medium align-middle"></i>Add Reminder Type
                                    </SpkButton>
                                </div>
                                <div className="d-flex" role="search">
                                    <Form.Control 
                                        className="me-2" 
                                        type="search" 
                                        placeholder="Search Reminder Type" 
                                        value={search}
                        ...
                        <Card.Body className="p-0">
                            <div className="table-responsive">
                                <SpkTables tableClass="text-nowrap" header={[{ title: 'ID' }, { title: 'Name' }, { title: 'Created Date' }, { title: 'Status' }, { title: 'Actions' }]}>
                                    {reminderTypesData.map((reminder, index) => (
                                        <tr key={index}>
                                            <td>{reminder.id}</td>
                                            <td>{reminder.name}</td>
                                            <td>{reminder.createdDate}</td>
                                            <td>
                                                <span className={`badge ${reminder.status === 'Active' ? 'bg-success-transparent' : 'bg-danger-transparent'}`}>
                                                    {reminder.status}
                                                </span>
                                            </td>
                                            <td>
