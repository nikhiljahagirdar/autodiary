...
const VehicleManufacturers = () => {
    ...
    return (
        <Fragment>
            <Seo title="Vehicle Manufacturers" />
            <Pageheader title="Admin" subtitle="Masters" currentpage="Vehicle Manufacturers" activepage="Vehicle Manufacturers" />
                            ...
                            <div className="d-flex align-items-center justify-content-between flex-wrap gap-3">
                                <div className="d-flex flex-wrap gap-1">
                                    <SpkButton Buttonvariant="primary" Customclass="btn me-2">
                                        <i className="ri-add-line me-1 fw-medium align-middle"></i>Add Manufacturer
                                    </SpkButton>
                                </div>
                                ...
                                <SpkTables tableClass="text-nowrap" header={[{ title: 'ID' }, { title: 'Name' }, { title: 'Created Date' }, { title: 'Status' }, { title: 'Actions' }]}>
                                    {manufacturersData.map((manufacturer, index) => (
                                        <tr key={index}>
                                            ...
                                            <td>
                                                <span className={`badge ${manufacturer.status === 'Active' ? 'bg-success-transparent' : 'bg-danger-transparent'}`}>
                                                    {manufacturer.status}
                                                </span>
                                            </td>
                                            <td>
