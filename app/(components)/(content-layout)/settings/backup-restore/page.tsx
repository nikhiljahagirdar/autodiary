...
const BackupRestore = () => {
    ...
    const backupRestoreData = [
        { id: 1, name: 'Backup #001', createdDate: '2024-01-15', status: 'Active' },
        { id: 2, name: 'Backup #002', createdDate: '2024-01-10', status: 'Active' },
        { id: 3, name: 'Backup #003', createdDate: '2024-01-08', status: 'Inactive' },
    ]

    return (
        <Fragment>
            <Seo title="Backup & Restore" />
            <Pageheader title="Settings" subtitle="Backup & Restore" currentpage="Backup & Restore" activepage="Backup & Restore" />
                            ...
                            <div className="d-flex align-items-center justify-content-between flex-wrap gap-3">
                                <div className="d-flex flex-wrap gap-1">
                                    <SpkButton Buttonvariant="primary" Customclass="btn me-2">
                                        <i className="ri-add-line me-1 fw-medium align-middle"></i>Create Backup
                                    </SpkButton>
                                </div>
                        ...
                        <Card.Body className="p-0">
                            <div className="table-responsive">
                                <SpkTables tableClass="text-nowrap" header={[{ title: 'ID' }, { title: 'Name' }, { title: 'Created Date' }, { title: 'Status' }, { title: 'Actions' }]}>
                                    {backupRestoreData.map((backup, index) => (
                                        <tr key={index}>
                                            <td>{backup.id}</td>
                                            <td>{backup.name}</td>
                                            <td>{backup.createdDate}</td>
                                            <td>
                                                <span className={`badge ${backup.status === 'Active' ? 'bg-success-transparent' : 'bg-danger-transparent'}`}>
                                                    {backup.status}
                                                </span>
                                            </td>
                                            <td>
