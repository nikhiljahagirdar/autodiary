  ...
  return (
    <Fragment>
      <Link href="#!" scroll={false} className={`side-menu__item ${MenuItems?.selected ? "active" : ""}`}
        ...
        <span className={`${level == 1 ? "side-menu__label" : "child-menu-item"}`}> {level > 1 && '• '}{MenuItems.title} {MenuItems.badgetxt ? (<span className={MenuItems.class}> {MenuItems.badgetxt} </span>
        ) : (
          ""
        )}
        </span>
      ...
      <ul className={`slide-menu child-menu child${level}  
          ...
          >
            {firstlevel?.type === "link" ?
              <Link href={firstlevel.path} className={`side-menu__item ${firstlevel?.selected ? 'active' : ''}`}>{firstlevel.icon}
                <span className="child-menu-item"> {level > 1 && '• '}{firstlevel.title} {firstlevel.badgetxt ? (<span className={firstlevel.class}> {firstlevel.badgetxt}</span>
                ) : (
                  ""
                )}
                </span>
            ...
            {firstlevel?.type === "empty" ?
              <Link href="#!" className={`side-menu__item`} onClick={handleClick}>
                {firstlevel.icon}
                <span className="child-menu-item">
                {level > 1 && '• '}{firstlevel.title}
                </span>
              </Link>
              : ""}
            {firstlevel?.type === "sub" ?
