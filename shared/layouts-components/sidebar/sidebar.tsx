...
const Sidebar = () => {
	...
	// Initialize with a default state that is consistent on server and client to prevent hydration errors.
	// The actual state is loaded client-side in useEffect.
	const [variable, setVariable] = useState({ dataVerticalStyle: '', dir: '' });

	useEffect(() => {
		// Set the state from the service once the component has mounted on the client.
		setVariable(getState());
	...
	return (
		<Fragment>
			...
			{/* <!-- Start::app-sidebar --> */}
			<aside className="app-sidebar sticky d-flex flex-column vh-100" id="sidebar" onMouseOver={Onhover} onMouseLeave={Outhover} >
				...
				<SimpleBar className="main-sidebar flex-grow-1  vh-100" id="sidebar-scroll">
					{/* <!-- Start::nav --> */}
					<nav className="main-menu-container nav nav-pills flex-column sub-open vh-100">
						<div className="slide-left" id="slide-left" onClick={slideLeft} >
							<svg xmlns="http://www.w3.org/2000/svg" fill="#7b8191" width="24" height="24" viewBox="0 0 24 24"> <path d="M13.293 6.293 7.586 12l5.707 5.707 1.414-1.414L10.414 12l4.293-4.293z"></path> </svg>
						</div>
						<ul className="main-menu flex-grow-1 vh-100" onClick={() => Sideclick()}>

							{/* <!-- Start::slide --> */}

							{MENUITEMS.map((list: any, index: any) => (
								<li key={index} className={` ${list.menutitle ? "slide__category" : ""} ${list.type === 'link' ? 'slide' : ''} ${list.type === 'sub' ? 'slide has-sub' : ''} ${list.active ? 'open' : ''}  ${list?.selected ? 'active' : ''}  `}>
										{list.menutitle ?
										...
										{list.type === "link" ?
											<Link href={list.path} className={`side-menu__item  ${list.selected ? 'active' : ''}`}>
												<span className={`${variable?.dataVerticalStyle == 'doublemenu' ? '' : 'd-none'}`}>
													<SpkTooltips placement="auto" title={list.title}>
														<div>{list.icon}</div>
													</SpkTooltips>
												</span>
												{variable.dataVerticalStyle != "doublemenu" ? list.icon : ""}
